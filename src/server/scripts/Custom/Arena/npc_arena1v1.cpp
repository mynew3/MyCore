...
public:
	...
	bool OnGossipHello(Player* player, Creature* me)
	{
		...
		{
			ChatHandler(player->GetSession()).SendSysMessage("1х1 отключено");
			return true;
		}

		if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Покинуть арену 1х1", GOSSIP_SENDER_MAIN, 3, "Вы уверены?", 0, false);
		else
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_3v3_5:30|t Не рейтинговая игра 1х1 ", GOSSIP_SENDER_MAIN, 20);

		if(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == NULL)
			player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Создать команду арены 1х1", GOSSIP_SENDER_MAIN, 1, "Вы действительно хотите создать команду арены 1х1?", sWorld->getIntConfig(CONFIG_ARENA_1V1_COSTS), false);
		else
		{
			if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_3v3_5:30|t Рейтинговая игра 1х1", GOSSIP_SENDER_MAIN, 2);
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Распустить команду арены?", GOSSIP_SENDER_MAIN, 5, "Вы уверены?", 0, false);
			}
		...
		case 1: // Create new Arenateam
			{
				...
				{
					ChatHandler(player->GetSession()).PSendSysMessage("Вам нужен уровень %u+ чтобы создать команду арены 1х1.", sWorld->getIntConfig(CONFIG_ARENA_1V1_MIN_LEVEL));
		...
		case 2: // Join Queue Arena (rated)
			{
				if(Arena1v1CheckTalents(player) && JoinQueueArena(player, me, true) == false)
					ChatHandler(player->GetSession()).SendSysMessage("Что то пошло не так во время присоединения к очереди.");
		...
		case 4: // get statistics
			{
				...
				if(at)
				{
					...
					s << "\nИгры в сезоне: " << at->GetStats().SeasonGames;
					s << "\nПобеды в сезоне: " << at->GetStats().SeasonWins;
					s << "\nИгры за неделю: " << at->GetStats().WeekGames;
					s << "\nПобеды за неделю: " << at->GetStats().WeekWins;
