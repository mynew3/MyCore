...
namespace
{
    ...
    public:
...
#endif
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУдалить всё искажение.", EQUIPMENT_SLOT_END + 2, 0, "Удалить искажениеs from all equipped items?", 0, false);
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
        {
            ...
            switch (sender)
            {
                ...
                case EQUIPMENT_SLOT_END + 1: // Main menu
                    ...
                    break;
                case EQUIPMENT_SLOT_END + 2: // Удалить искажениеs
                {
                    bool removed = false;
                    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
                    {
                ...
                case EQUIPMENT_SLOT_END + 3: // Удалить искажение from single item
                {
                ...
                case EQUIPMENT_SLOT_END + 4: // Presets menu
                {
                    ...
                    if (!player->presetMap.empty())
                    {
                        ...
                        if (player->presetMap.size() < sTransmogrification->MaxSets)
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохронить комплект.", EQUIPMENT_SLOT_END + 8, 0);
                    }
                    else
                ...
                case EQUIPMENT_SLOT_END + 6: // view preset
                {
                    ...
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tИспользовать комплект.", EQUIPMENT_SLOT_END + 5, action, "Using this set for transmogrify will bind transmogrified items to you and make them non-refundable and non-tradeable.\nDo you wish to continue?\n\n" + it->second.name, 0, false);
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tУдалить комплект.", EQUIPMENT_SLOT_END + 7, action, "Are you sure you want to delete " + it->second.name + "?", 0, false);
                ...
                case EQUIPMENT_SLOT_END + 8: // Save preset
                {
                    ...
                    if (canSave)
                        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tSave set", 0, 0, "Введите название комплекта.", cost*sTransmogrification->SetCostModifier + sTransmogrification->SetCopperCost, true);
        ...
        void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
        {
            ...
            if (oldItem)
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sTransmogrification->GetFakeEntry(oldItem) == newItem->GetEntry())
                        continue;
                    ++limit;
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sTransmogrification->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sTransmogrification->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Вы точно хотите это сделать? После этого действия предмет станет персональный!" + sTransmogrification->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sTransmogrification->GetItemLink(newItem, session) + ss.str(), price, false);
                }
            ...
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУдалить искажение", EQUIPMENT_SLOT_END + 3, slot, "Удалить искажение from the slot?", 0, false);
