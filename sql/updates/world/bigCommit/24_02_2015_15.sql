...
(67039, 'EC_spell_vanity_mount_check');
...
UPDATE `creature_template` SET `gossip_menu_id`=@GOSSIPSQUIRE, `ScriptName`='EC_npc_vanity_argent_squire' WHERE `entry` = @SQUIRE;
UPDATE `creature_template` SET `gossip_menu_id`=@GOSSIPGRUNT, `ScriptName`='EC_npc_vanity_argent_squire' WHERE `entry` = @GRUNT;
